...
def make_data(n=500,severity=0.0):
    ...
    for i in range(n):
        ...
        if severity>0:
            x=[(1-0.5*severity)*v + severity*0.4*math.sin(j+1) + random.gauss(0,0.15+0.2*severity) for j,v in enumerate(x)]
...
def tune(xs,ys,g,idx,steps=120,lr=0.025):
    q=g[:]
    for _ in range(steps):
        for i in idx:
            grad=0.0
            for x,y in zip(xs[:180],ys[:180]):
...
def recovery(a_src,a_shift,a_new):
    return (a_new-a_shift)/max(a_src-a_shift,1e-12)
...
for sev in [0.2,0.4,0.6]:
    ...
    for frac in [0.1,0.25,0.5]:
        ...
        gl=tune(sx,sy,base,list(range(DIM)),steps=150)
        lc=tune(sx,sy,base,idx,steps=110)
        te,se=th_sign(sx[:180],sy[:180],base)
        tl,sl=th_sign(sx[:180],sy[:180],lc)
        tg,sg=th_sign(sx[:180],sy[:180],gl)
...
summary={'topic':'meta_adapt_phase2','rows':len(rows),'best_tradeoff':{k:best[k] for k in ['severity','fraction','strategy']},
'gate_signals':['fraction_scan','severity_scan','strategy_pareto']}
