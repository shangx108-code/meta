...
class TestTopic1SweepVariation(unittest.TestCase):
    def test_sweep_not_identical(self):
        cfg = {
            "seed": 19,
            "image_size": 16,
            "batch_size": 8,
            ...
            "robust_weight": 0.2,
            "output_dir": "outputs/topic1/test_pilot_variation",
        }
        cfg_path = Path("outputs/topic1/test_pilot_variation_cfg.json")
        cfg_path.parent.mkdir(parents=True, exist_ok=True)
        cfg_path.write_text(json.dumps(cfg), encoding="utf-8")
        run_pilot(str(cfg_path))
        with open("outputs/topic1/test_pilot_variation/pilot_sweep.csv", "r", encoding="utf-8") as f:
